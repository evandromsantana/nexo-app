...
import { getReceivedProposals, updateProposalStatus, acceptProposal, completeProposal, getUsersByIds } from '../../api/firestore.ts';
...
type ReceivedProposalsScreenProps = NativeStackScreenProps<RootStackParamList, 'ProposalsMain'>;

const ReceivedProposalsScreen = ({ navigation }: ReceivedProposalsScreenProps) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    setLoading(true);
    const unsubscribe = getReceivedProposals(user.uid, async (receivedProposals) => {
      try {
        if (receivedProposals.length > 0) {
          const senderIds = [...new Set(receivedProposals.map(p => p.senderId))];
          const usersMap = await getUsersByIds(senderIds);

          const enrichedProposals = receivedProposals.map(proposal => ({
            ...proposal,
            senderEmail: usersMap.get(proposal.senderId)?.email || 'E-mail não encontrado',
          }));
      ...
      Alert.alert("Erro", "Não foi possível carregar as propostas recebidas em tempo real.");
      setLoading(false);
    });
  ...
  return (
    <View style={styles.container}>
      {proposals.length === 0 ? (
        <View style={styles.centered}><Text>Nenhuma proposta recebida.</Text></View>
      ) : (
        <FlatList
          data={proposals}
          ...
          renderItem={({ item }: { item: Proposal }) => (
            <View style={styles.proposalItem}>
              <Text style={styles.proposalText}>De: {item.senderEmail || 'Usuário desconhecido'}</Text>
              ...
              {item.status === 'pending' && (
                <View style={styles.buttonContainer}>
                  <AppButton title="Aceitar" onPress={() => handleAcceptProposal(item.id)} variant="secondary" style={styles.flexButton} />
                  <AppButton title="Rejeitar" onPress={() => handleRejectProposal(item.id)} variant="danger" style={styles.flexButton} />
                </View>
              )}
